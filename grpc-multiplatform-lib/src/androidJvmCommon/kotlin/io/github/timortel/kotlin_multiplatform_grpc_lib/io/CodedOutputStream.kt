...
actual class CodedOutputStream(private val impl: CodedOutputStream) {
    ...
    private fun <T> writeArray(
        stream: io.github.timortel.kotlin_multiplatform_grpc_lib.io.CodedOutputStream,
        fieldNumber: Int,
        values: Collection<T>,
        tag: UInt,
        ...
        writeNoTag: (T) -> Unit,
        writeTag: (Int, T) -> Unit
    ) = io.github.timortel.kotlin_multiplatform_grpc_lib.io.writeArray(
        stream,
        fieldNumber,
        values,
        tag,
        { computeSizeNoTag(it).toULong() },
        writeNoTag,
        writeTag
    )
